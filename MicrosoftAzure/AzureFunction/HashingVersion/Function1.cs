...
namespace FunctionApp1
{
	public static class Function1
	{
		...
		static string subscriptionKey = "884a6e4f5cc74558a32d5c759dd43e70";
		static string subscriptionEndpoint = "https://apim.projectwabash.com/photodna/v1.0/MatchEdgeHash";
		static string senderEmail = "jnnortz@gmail.com";
		static string receiverEmail = "jnnortz@gmail.com";
		static string callbackEndpoint = "https://requestb.in/1d2ti1q1";
		...
		public static async void Run([BlobTrigger("allyispictureuploadfolder/{name}.{ext}", Connection = "AzureWebJobsStorage")]Stream input, string name, string ext, TraceWriter log)
		{
			...
			var hashed = PdnaClientHash.GenerateHash(input);

			await MakeRequest(hashed, name, ext, log);
		...
		public static async Task MakeRequest(byte[] input, string name, string ext, TraceWriter log)
		{
			try
			{
				...
				dynamic json =  new RequestObject();
				json.formdata = new Dictionary<string, string>[1];
				json.formdata[0] = new Dictionary<string, string>();
				json.formdata[0].Add("name", name);
				json.formdata[0].Add("type", "file");
				json.formdata[0].Add("value", input.ToString());
				string contents = "";
